...
export async function generateMetadata({ params }: { params: Promise<{ id: string }> }): Promise<Metadata> {
  ...
  return screening ? { title: `${screening.name} overview | HealthScope AI`, description: screening.overview } : {};
}
...
export default async function ScreeningOverview({ params }: { params: Promise<{ id: string }> }) {
  ...
  return <main className="detail-page" style={{ "--detail-accent": screening.accent } as React.CSSProperties}>
    <header className="topbar"><Link className="brand" href="/"><span className="brand-mark"><HeartPulse size={20}/></span>HealthScope<span>AI</span></Link><div className="privacy-pill"><LockKeyhole size={15}/> Educational use only</div></header>
    ...
    <footer><div className="footer-warning"><ShieldCheck size={20}/><p><strong>Educational use only.</strong> Symptoms require professional assessment. This model cannot diagnose, rule out disease, or replace emergency care.</p></div><p>HealthScope AI · Built with privacy and clarity in mind.</p></footer>
  </main>;
}
